...
public class ReadMangaOnlineActivity extends FragmentActivity {
	...
	private class ImageAdapter extends PagerAdapter {
		...
		public Object instantiateItem(ViewGroup view, int position) {
			...
			ImageLoader.getInstance().displayImage(
					arrImg.get(position).toString(), imageView, options,
	...
	protected void onDestroy() {
		// TODO Auto-generated method stub
		//Toast.makeText(getBaseContext(),""+ positions,Toast.LENGTH_LONG).show();
		if(positions== 1 || positions == arrImg.size()-1){}
		else{
		ContentValues values = new ContentValues();
		String chap =link.substring(link.lastIndexOf('/') + 1);
		values.put("Chap",chap);
		values.put("pager",String.valueOf(positions-1));
		database.insert("tblResume", null, values);
		}
		super.onDestroy();
	}
