	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		listView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> a, View v, int position,
				...
				ListMangaItems newsData = (ListMangaItems) listView
						.getItemAtPosition(position);

				Intent intent = new Intent(getActivity(),
						MangaInforActivity.class);
				intent.putExtra("position", arrayMangas.get(position)
						.getPosition());
		...
		protected Void doInBackground(Void... params) {
			...
			try {
				...
				for (int i = 0; i < nListNews.getLength(); i++) {
					...
					String tittle = ((Element) n1News)
							.getElementsByTagName("title").item(0)
							.getTextContent();
					String img = ((Element) n1News).getElementsByTagName("img")
							.item(0).getTextContent();
					String authors = ((Element) n1News)
							.getElementsByTagName("authors").item(0)
							.getTextContent();
